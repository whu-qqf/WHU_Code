...
{
	int i, MsgLen, MsgId, Status=0;  // 记录待处理数据的起始位置
	EPOCHOBSDATA tmpobs;
	i = 0;
	while (1)
	{
		...
		{
		case 43:
			DecodeRange(buff + i, MsgLen, &tmpobs);
			for (int i = 0; i < tmpobs.SatNum; i++)tmpobs.SatObs[i].Valid = true;
			if (obs->SatObs[0].Prn == 0) *obs = tmpobs;
			else
			{
				for(int i=0;i<tmpobs.SatNum;i++)
				{
					//if(tmpobs.SatObs[i].System==GPS)tmpobs.SatObs[i].Valid = false;
					if (tmpobs.SatObs[i].half[0] == 0 || tmpobs.SatObs[i].half[1] == 0)
					{
						tmpobs.SatObs[i].Valid = false;
						continue;
					}
					for(int j=0;j<obs->SatNum;j++)
					{
						if (obs->SatObs[j].Prn == tmpobs.SatObs[i].Prn && obs->SatObs[j].System == tmpobs.SatObs[i].System)
						{
							if (tmpobs.SatObs[i].LockTime[0] <= obs->SatObs[j].LockTime[0] || tmpobs.SatObs[i].LockTime[1] <= obs->SatObs[j].LockTime[1])tmpobs.SatObs[i].Valid = false;
							break;
						}
					}
				}
				memset(obs->SatObs, 0, MAXCHANNUM * sizeof(SATOBSDATA));
				obs->SatNum = tmpobs.SatNum;
				for (int i = 0; i < tmpobs.SatNum; i++)obs->SatObs[i] = tmpobs.SatObs[i];
