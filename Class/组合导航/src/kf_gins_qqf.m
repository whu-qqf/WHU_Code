...
if cfg.usezupt
    refdata = importdata("dataset_qqf\REF_NAV.nav");
    refindex = 1;
    refdata = refdata(refdata(:, 2) >= cfg.starttime, :);
...
%% initialization 
[kf, navstate] = Initialize_qqf(cfg);
laststate = navstate;
...
for imuindex = 2:size(imudata, 1)-1
    ...
    %%
    if cfg.usezupt
        if refindex == 1 && refdata(refindex,2)>=thisimu(1,1)
            is_zerovelocity = true;
        elseif refindex>size(refdata,1)
             is_zerovelocity = false;
        else
            ...
            if refindex <= size(refdata,1)
                v1 = sqrt(refdata(refindex-1,6)^2+refdata(refindex-1,7)^2+refdata(refindex-1,8)^2);
                v2 = sqrt(refdata(refindex,6)^2+refdata(refindex,7)^2+refdata(refindex,8)^2);
                if abs((v1+v2)/2)<0.005
                    is_zerovelocity = true;
                end
            end
        end
    ...
    if is_zerovelocity
        kf = ZUPTUpdate(navstate, laststate, kf, thisimu,is_zerovelocity,imudt);
        [kf, navstate] = ErrorFeedback(kf, navstate);
    end
    ...
    if cfg.useodonhc
        ...
        if (thisimu(1, 1) >= odoupdatetime)
            ...
            if valid
                ...
                kf = ODONHCUpdate_qqf(navstate, odonhc_vel, kf, cfg, thisimu, imudt);
                [kf, navstate] = ErrorFeedback(kf, navstate);
            end
